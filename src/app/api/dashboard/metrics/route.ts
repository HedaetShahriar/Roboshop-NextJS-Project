...
function parseDate(value: string | null, fallback: Date): Date {
  ...
  return !d || isNaN(d.getTime()) ? fallback : d;
}
...
export async function GET(req: NextRequest): Promise<Response> {
  try {
    ...
    const [revenueByDay, ordersByDay] = await Promise.all([
      db
        ...
        .aggregate<MetricPoint>([
          ...
          {
            $group: {
              _id: { $dateToString: dateFmt },
              value: { $sum: { $toDouble: "$amounts.total" } },
            },
          },
    ...
    const totals = {
      revenue: Number(
        revenueByDay.reduce((s, d) => s + Number(d.value || 0), 0).toFixed(2)
      ),
