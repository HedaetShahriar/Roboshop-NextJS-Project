...
interface ImportRow {
  ...
  image?: string | number;
  price?: string | number;
  has_discount_price?: boolean | string | number;
...
export async function POST(
  ...
  try {
    ...
    for (const r of rows) {
      ...
      const hasDiscount =
        r.has_discount_price === true ||
        r.has_discount_price === "true" ||
        r.has_discount_price === "on" ||
        r.has_discount_price === 1;
      const discountPrice = Number(r.discount_price || 0);
      const currentStock = Number.isFinite(Number(r.current_stock))
    ...
    if (!docs.length) {
      return NextResponse.json(
        { ok: false, error: "No valid rows after validation" },
        { status: 400 }
      );
    }
    ...
    const result: BulkWriteResult = await db
      .collection("products")
    ...
    return NextResponse.json(
      { ok: false, error: "Failed to parse or import file" },
      { status: 500 }
    );
  }
}
