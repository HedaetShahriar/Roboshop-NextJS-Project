...
const SUGGESTED_AREAS: Record<string, string[]> = {
...
const KNOWN_PROMOS: Record<string, PromoInfo> = {
...
function OptionTile({ active, onClick, icon: Icon, label, sub }: OptionTileProps) {
  return (
    <button
      type="button"
      onClick={onClick}
...
export default function CheckoutPage() {
  ...
  const [showAddressMore, setShowAddressMore] = useState<boolean>(() => {
  ...
  const onChangePhoneLocal = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    async function prefill() {
      try {
        ...
        if (addrRes.ok) {
          ...
          const preferred = addresses.find((a: { label?: string }) => (a.label || "").toLowerCase() === "home") || addresses[0];
  ...
  useEffect(() => {
    ...
    async function syncCart() {
      ...
      try {
        ...
        const byIdQty = (arr: Array<{ id?: string; qty?: number }>) => {
  ...
  const onChange = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setForm((f) => {
      const next = { ...f, [name]: v } as CheckoutForm;
  ...
  const validateAddressForm = (f: CheckoutForm): FormErrors => {
    const errs: FormErrors = {};
  ...
  const placeOrder = async (e: FormEvent<HTMLFormElement> | React.MouseEvent) => {
    e.preventDefault();
    setMessage(null);
    const errs = validate();
