...
export default async function AdminActivityPage({ searchParams }: AdminActivityPageProps) {
  ...
  const limit = Math.min(200, Math.max(20, Number(params?.limit || 100)));
  const logs = await db.collection<AuditLog>('audit_logs').find({}).sort({ createdAt: -1 }).limit(limit).toArray();
  return (
