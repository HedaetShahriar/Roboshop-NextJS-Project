...
export default async function AdminOrderDetail({ params }: AdminOrderDetailProps) {
  ...
  let id: ObjectId;
  try { id = new ObjectId(resolvedParams.id); } catch { return notFound(); }
  const order = await db.collection<Order>('orders').findOne({ _id: id });
