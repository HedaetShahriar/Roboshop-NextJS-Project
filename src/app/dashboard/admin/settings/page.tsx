...
interface PlatformSettings {
  ...
  ui: {
    ...
    navbarTransparentOnHome: boolean;
    showFooterSocial: boolean;
    footerText: string;
  };
  payments: { provider: string; publicKey: string; secretKey: string };
  emails: {
...
async function getSettings(): Promise<PlatformSettings> {
  ...
  const doc = await db.collection('settings').findOne({ _id: 'platform' as unknown as import("mongodb").ObjectId });
  return (doc as unknown as PlatformSettings) || {
    _id: 'platform',
    siteName: 'Roboshop',
...
async function savePlatform(formData: FormData) {
  ...
  const db = await getDb();
  await db.collection('settings').updateOne({ _id: 'platform' as unknown as import("mongodb").ObjectId }, { $set: { siteName, supportEmail } }, { upsert: true });
...
async function saveBranding(formData: FormData) {
  ...
  await db.collection('settings').updateOne(
    { _id: 'platform' as unknown as import("mongodb").ObjectId },
...
async function saveUI(formData: FormData) {
  'use server';
  const session = await getServerSession(authOptions);
  if (session?.user?.role !== 'admin') return;
...
async function saveSEOAdvanced(formData: FormData) {
  'use server';
  const session = await getServerSession(authOptions);
  if (session?.user?.role !== 'admin') return;
...
async function saveCheckoutAdvanced(formData: FormData) {
  'use server';
  const session = await getServerSession(authOptions);
  if (session?.user?.role !== 'admin') return;
...
async function saveNavigation(formData: FormData) {
  ...
  const session = await getServerSession(authOptions);
  if (session?.user?.role !== 'admin') return;
  const headerLinks = parseJsonArray(String(formData.get('headerLinks') || '[]')) as NavigationLink[];
  const footerLinks = parseJsonArray(String(formData.get('footerLinks') || '[]')) as NavigationLink[];
  const socialLinks = parseJsonArray(String(formData.get('socialLinks') || '[]')) as NavigationLink[];
  const db = await getDb();
  await db.collection('settings').updateOne(
    { _id: 'platform' as unknown as import("mongodb").ObjectId },
...
async function saveCommerce(formData: FormData) {
  'use server';
  const session = await getServerSession(authOptions);
  if (session?.user?.role !== 'admin') return;
...
async function savePayments(formData: FormData) {
  ...
  const db = await getDb();
  await db.collection('settings').updateOne({ _id: 'platform' as unknown as import("mongodb").ObjectId }, { $set: { payments: { provider, publicKey, secretKey } } }, { upsert: true });
  redirect('/dashboard/admin/settings#payments');
}

async function saveEmails(formData: FormData) {
  'use server';
  const session = await getServerSession(authOptions);
  if (session?.user?.role !== 'admin') return;
  ...
  const db = await getDb();
  await db.collection('settings').updateOne({ _id: 'platform' as unknown as import("mongodb").ObjectId }, { $set: { emails: { orderConfirmation, shipmentUpdate } } }, { upsert: true });
