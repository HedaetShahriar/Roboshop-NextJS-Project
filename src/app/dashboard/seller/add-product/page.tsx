...
import { useEffect, useMemo, useRef, useState, useTransition, ChangeEvent, FormEvent } from "react";
...
export default function AddProductPage() {
  ...
  const [message, setMessage] = useState<AddProductResult | null>(null);
  ...
  const [slugInfo, setSlugInfo] = useState<SlugInfo>({ checking: false, available: null });
  ...
  const handleUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (json?.ok && json.url) {
        const input = formRef.current?.querySelector('#image') as HTMLInputElement | null;
  ...
  const handleImageUrlChange = (e: ChangeEvent<HTMLInputElement>) => setImgPreview(e.target.value);
