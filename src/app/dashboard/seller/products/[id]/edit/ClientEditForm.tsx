...
import { useActionState, useEffect, useMemo, useState, useCallback, useRef, ChangeEvent, KeyboardEvent, FormEvent } from "react";
...
interface ProductData {
  ...
  sku?: string;
  image?: string;
  gallery?: Array<string | { url: string; alt: string }>;
  category?: string;
  subcategory?: string;
  price?: number | string;
...
type TabId = 'basic' | 'media' | 'details' | 'variants' | 'markets';

export default function ClientEditForm({ product, action }: ClientEditFormProps) {
  ...
  const [slugStatus, setSlugStatus] = useState<SlugStatus>({ checking: false, available: true });
  ...
  const [discountMode, setDiscountMode] = useState<'none' | 'percent' | 'price'>(product.has_discount_price ? 'price' : 'none');
  const [percentVal, setPercentVal] = useState(10);
  const [variantSummary, setVariantSummary] = useState<VariantSummary>({ totalStock: 0, dupSkus: [] });
  ...
  const onSubcategoryChange = useCallback((e: ChangeEvent<HTMLSelectElement>) => {
  ...
  const onHasDiscountChange = useCallback((e: ChangeEvent<HTMLInputElement>) => {
  ...
  const onPriceInput = useCallback((e: ChangeEvent<HTMLInputElement>) => {
    const n = Number(e.target.value || 0);
    setPricePreview(formatBDT(n));
    setPriceNum(n);
  }, []);
  const onDiscountInput = useCallback((e: ChangeEvent<HTMLInputElement>) => {
    const n = Number(e.target.value || 0);
  ...
  const applyFixed = useCallback((value: number) => {
    const price = priceRef.current ? Number(priceRef.current.value || 0) : priceNum;
  ...
  const TABS: [TabId, string][] = useMemo(() => ([
    ['basic','Basic'],
    ['media','Media'],
    ['details','Details'],
    ['variants','Variants'],
    ...
    <form action={formAction} onSubmit={handleSubmit} onChange={onAnyChange} className="space-y-2 pb-6" aria-busy={pending}>
                ...
                <AccordionItem value="image" className="border-none">
                  ...
                  <AccordionContent>
                    ...
                    <div className="mt-2 flex items-center gap-2">
                      {Array.from(new Set([imageUrl, ...((product.gallery||[]).map(g => typeof g === 'string' ? g : g.url).filter(Boolean))]))
                        .filter(Boolean)
                        .slice(0,4)
                        .map((url)=> (
      ...
      {tab === 'media' && (
        <div id="panel-media" role="tabpanel" aria-labelledby="tab-media" className="rounded-md border bg-white p-3 md:p-4">
          ...
          <div className="mt-1">
            <GalleryEditor name="gallery" initial={(product.gallery || []).map(g => typeof g === 'string' ? { url: g, alt: '' } : g)} />
          </div>
        </div>
      )}
