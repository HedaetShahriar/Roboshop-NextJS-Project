...
export default async function ProductViewPage({ params }: ProductViewPageProps) {
  ...
  <ProductGalleryView images={[p.image, ...gallery].filter((img): img is string => Boolean(img))} name={p.name || ''} slug={p.slug || ''} />
                  ...
                  <div className="rounded border bg-white">
                    <div className="divide-y">
                      {Array.isArray(p.specs) ? (
                        ...
                        Object.keys(p.specs).length > 0 ? Object.entries(p.specs as Record<string, string>).map(([k,v]) => (
