...
interface SerializedOrder extends Omit<OrderDocument, '_id'> {
  _id: string;
}
...
interface SerializedIssue extends Omit<OrderIssue, '_id' | 'orderId'> {
  _id: string;
  orderId: string;
}
...
export default async function OrderDetailPage({ params }: OrderDetailPageProps) {
  ...
  let order: SerializedOrder;
  try {
    const doc = await db.collection<OrderDocument>("orders").findOne({ _id: new ObjectId(id), userId: session.user.email });
  ...
  const issues = await db
    .collection<OrderIssue>('order_issues')
    .find({ orderId: new ObjectId(order._id) })
    .sort({ createdAt: -1 })
    .toArray();
  const issueList: SerializedIssue[] = issues.map((i) => ({ ...i, _id: i._id.toString(), orderId: i.orderId.toString() }));
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white border rounded p-4">
            ...
            {/* Timeline */}
            {(() => {
              const history: HistoryItem[] = (order.history && order.history.length > 0)
                ? [...order.history]
              ...
              const formatDate = (d: Date | string) => formatDateTime(d);
              return (
                <div className="mt-4">
            ...
            {order.status === 'processing' && (
              <form
                action={async () => {
                  ...
                  await (db.collection('orders') as any).updateOne({ _id: doc._id }, { $set: { status: 'cancelled', updatedAt: new Date() }, $push: { history: { code: 'cancelled', label: 'Order cancelled', at: new Date() } } });
...
async function ReportIssueForm({ orderId }: ReportIssueFormProps) {
  async function createIssue(data: FormData) {
    ...
    if (!existing) {
      ...
      await (db.collection('orders') as any).updateOne({ _id: order._id }, { $push: { history: { code: 'issue-opened', label: 'Issue reported', at: now } }, $set: { updatedAt: now } });
    }
...
async function AddIssueMessageForm({ issueId }: AddIssueMessageFormProps) {
  async function addMessage(data: FormData) {
    'use server';
    const session = await getServerSession(authOptions);
    if (!session?.user?.email) return;
    const db = await getDb();
    ...
    if (!text) return;
    const now = new Date();
    await (db.collection('order_issues') as any).updateOne({ _id }, { $push: { messages: { by: 'customer', text, at: now } }, $set: { updatedAt: now } });
