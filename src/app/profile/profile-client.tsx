...
export default function ProfileClient({ initialProfile, initialAddresses }: ProfileClientProps) {
  ...
  const [profile, setProfile] = useState<Profile>(initialProfile || { name: "", email: "", phone: "", image: "" });
  ...
  async function compressImage(
    file: File,
    { maxSize = 512, quality = 0.7, type = 'image/webp' }: { maxSize?: number; quality?: number; type?: string } = {}
  ): Promise<Blob> {
    ...
    const blob = await new Promise<Blob | null>((resolve) => canvas.toBlob((b) => resolve(b), outType, quality));
    // Fallback to image/jpeg if WEBP not supported
    if (!blob) {
      const jpegBlob = await new Promise<Blob | null>((resolve) => canvas.toBlob((b) => resolve(b), 'image/jpeg', quality));
      return jpegBlob!;
    }
    return blob;
  }
            ...
            <CardContent className="grid md:grid-cols-[auto_1fr] gap-6 items-start">
              ...
              <div className="grid gap-4">
                <div className="grid gap-2">
                  ...
                  <Input id="name" value={profile.name} onChange={(e: ChangeEvent<HTMLInputElement>) => setProfile((p) => ({ ...p, name: e.target.value }))} />
                </div>
                <div className="grid gap-2">
                ...
                <div className="grid gap-2">
                  ...
                  <Input id="phone" value={profile.phone} onChange={(e: ChangeEvent<HTMLInputElement>) => setProfile((p) => ({ ...p, phone: e.target.value }))} />
                </div>
              ...
              <CardContent>
                <div className="grid md:grid-cols-3 gap-4">
                  <div className="grid gap-2">
                    ...
                    <Input id="label" value={addrForm.label} onChange={(e: ChangeEvent<HTMLInputElement>) => setAddrForm((f) => ({ ...f, label: e.target.value }))} />
                  </div>
                  <div className="grid gap-2">
                    <Label htmlFor="country">Country</Label>
                    <Input id="country" value={addrForm.country} onChange={(e: ChangeEvent<HTMLInputElement>) => setAddrForm((f) => ({ ...f, country: e.target.value }))} />
                  </div>
                  <div className="grid gap-2">
                    <Label htmlFor="city">City</Label>
                    <Input id="city" value={addrForm.city} onChange={(e: ChangeEvent<HTMLInputElement>) => setAddrForm((f) => ({ ...f, city: e.target.value }))} />
                  </div>
                </div>
                <div className="grid md:grid-cols-3 gap-4 mt-4">
                  <div className="grid gap-2">
                    <Label htmlFor="area">Area</Label>
                    <Input id="area" value={addrForm.area} onChange={(e: ChangeEvent<HTMLInputElement>) => setAddrForm((f) => ({ ...f, area: e.target.value }))} />
                  </div>
                  <div className="grid gap-2">
                    <Label htmlFor="address1">Address Line 1</Label>
                    <Input id="address1" value={addrForm.address1} onChange={(e: ChangeEvent<HTMLInputElement>) => setAddrForm((f) => ({ ...f, address1: e.target.value }))} />
                  </div>
                ...
                <div className="grid md:grid-cols-3 gap-4 mt-4">
                  <div className="grid gap-2">
                    ...
                    <Input id="postalCode" value={addrForm.postalCode} onChange={(e: ChangeEvent<HTMLInputElement>) => setAddrForm((f) => ({ ...f, postalCode: e.target.value }))} />
                  </div>
                </div>
