...
function xml(items: SitemapItem[]): string {
...
export async function GET(): Promise<NextResponse> {
  ...
  const add = (loc: string, meta: Partial<SitemapItem> = {}) => {
    urls.set(loc, { loc, ...meta });
  };
  ...
  // Products (from DB)
  const products = await db.collection<ProductDoc>('products').find(
