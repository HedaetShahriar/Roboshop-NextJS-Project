...
function BarChartBase({
  ...
  const formatter = useMemo(
    () => new Intl.DateTimeFormat(locale, dateFormatOptions),
    [locale, dateFormatOptions]
  );
  const formatDate = (d: Date) => formatter.format(d);
  const { bars, max } = useMemo(
    () => normalize(data, width, height, 24, formatDate),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [data, width, height, formatter]
  );
  ...
  const linePath = useMemo(() => {
    ...
    return bars
      .map((b, i) => `${i === 0 ? "M" : "L"} ${b.x + b.width / 2} ${b.y}`)
      .join(" ");
  }, [bars, showLine]);
    ...
    >
        ...
        {bars.map((b, i) => (
          <rect
            ...
            onMouseMove={(e: MouseEvent) =>
              setHover({ i, b, clientX: e.clientX, clientY: e.clientY })
            }
      ...
      {bars.length > 0 && (
        <g fill="#6b7280" fontSize="10">
          <text x="24" y={height - 8}>
            {bars[0].label}
          </text>
          <text x={width - 24} y={height - 8} textAnchor="end">
      ...
      {tooltip && hover && (
        <g>
          ...
          <text
            x={hover.b.x + 6}
            y={Math.max(hover.b.y - 12, 12)}
            fill="#fff"
            fontSize="11"
          >
            {hover.b.label}:{" "}
            {valueFormatter ? valueFormatter(hover.b.value) : hover.b.value}
          </text>
        </g>
      )}
    </svg>
