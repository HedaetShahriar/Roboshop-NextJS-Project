...
import { useCallback, useRef, useState, type DragEvent, type ClipboardEvent, type ChangeEvent, type KeyboardEvent } from "react";
...
export default function GalleryEditor({ name = "gallery", initial = [] }: GalleryEditorProps) {
  const [items, setItems] = useState<GalleryItem[]>(Array.isArray(initial) ? initial : []);
  const [uploading, setUploading] = useState(false);
  const fileRef = useRef<HTMLInputElement>(null);
  const [dragging, setDragging] = useState<number | null>(null);

  const add = useCallback(() => setItems((r) => [...r, { url: "", alt: "" }]), []);
  const remove = useCallback((idx: number) => setItems((r) => r.filter((_, i) => i !== idx)), []);
  const update = useCallback((idx: number, field: keyof GalleryItem, val: string) => setItems((r) => r.map((it, i) => i === idx ? { ...it, [field]: val } : it)), []);
  const move = useCallback((idx: number, dir: number) => setItems((r) => {
    const next = r.slice();
    const j = idx + dir;
  ...
  const onFilePicker = async (e: ChangeEvent<HTMLInputElement>) => {
