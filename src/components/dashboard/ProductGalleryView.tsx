...
export default function ProductGalleryView({ images = [], name = "", slug = "" }: ProductGalleryViewProps) {
  ...
  const needsUnoptimized = (url: string) => !!url && url.startsWith('http://');
  const [heroNoOpt, setHeroNoOpt] = useState(needsUnoptimized(hero));
  const [thumbNoOpt, setThumbNoOpt] = useState<Set<number>>(() => new Set());

  const markThumbNoOpt = (i: number) => setThumbNoOpt((s) => { const n = new Set(s); n.add(i); return n; });
