...
import { useCallback, useState, useEffect, type DragEvent, type ChangeEvent, type KeyboardEvent } from "react";
...
interface VariantsEditorProps {
  name?: string;
  initial?: Variant[];
  onSummaryChange?: (summary: VariantsSummary) => void;
}

export default function VariantsEditor({ name = "variants", initial = [], onSummaryChange }: VariantsEditorProps) {
  const [rows, setRows] = useState<Variant[]>(Array.isArray(initial) ? initial : []);
  ...
  const update = useCallback(<K extends keyof Variant>(idx: number, field: K, val: Variant[K]) => setRows((r) => r.map((it, i) => i === idx ? { ...it, [field]: val } : it)), []);

  // DnD
  const [dragging, setDragging] = useState<number | null>(null);

  const onDragStartRow = (e: DragEvent<HTMLDivElement>, idx: number) => {
  ...
  const onDragEnterRow = (e: DragEvent<HTMLDivElement>, overIdx: number) => {
    e.preventDefault();
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-[1fr_1fr_auto] gap-2 items-end rounded-md border p-3 bg-zinc-50">
        <Input placeholder="Sizes (comma separated)" value={sizeOptions} onChange={(e: ChangeEvent<HTMLInputElement>) => setSizeOptions(e.target.value)} />
        <Input placeholder="Colors (comma separated)" value={colorOptions} onChange={(e: ChangeEvent<HTMLInputElement>) => setColorOptions(e.target.value)} />
      ...
      <div className="space-y-2">
        ...
        {rows.map((it, idx) => (
          ...
          >
            ...
            <Input placeholder="Size" aria-label="Size" value={it.size} onChange={(e: ChangeEvent<HTMLInputElement>) => update(idx, 'size', e.target.value)} />
            <Input placeholder="Color" aria-label="Color" value={it.color} onChange={(e: ChangeEvent<HTMLInputElement>) => update(idx, 'color', e.target.value)} />
            <Input placeholder="SKU" aria-label="SKU" value={it.sku} onChange={(e: ChangeEvent<HTMLInputElement>) => update(idx, 'sku', e.target.value)} />
            <Input type="number" placeholder="Price delta" aria-label="Price delta" value={it.priceDelta} onChange={(e: ChangeEvent<HTMLInputElement>) => update(idx, 'priceDelta', Number(e.target.value))} />
            <Input type="number" placeholder="Stock" aria-label="Stock" value={it.stock} onChange={(e: ChangeEvent<HTMLInputElement>) => update(idx, 'stock', Number(e.target.value))} />
