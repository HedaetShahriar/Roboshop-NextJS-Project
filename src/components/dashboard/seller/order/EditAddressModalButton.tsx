...
interface EditAddressModalButtonProps {
  orderId: string;
...
export default function EditAddressModalButton({ 
  orderId, 
  initialAddress = {}, 
  action, 
  label = "Edit", 
  disabled = false 
}: EditAddressModalButtonProps) {
  ...
  return (
    <>
  <Button type="button" size="sm" variant="secondary" onClick={handleButtonClick} disabled={disabled}>{label}</Button>
      {open && (
        <div className="fixed inset-0 z-[60]">
          ...
          <div className="absolute left-1/2 top-1/2 w-[92vw] max-w-lg -translate-x-1/2 -translate-y-1/2 rounded-lg border bg-white shadow-xl">
            <div className="px-4 py-3 border-b flex items-center justify-between">
              ...
              <button className="text-sm text-muted-foreground hover:text-foreground" onClick={handleCloseClick} aria-label="Close">✕</button>
            </div>
            <form action={handleFormAction}>
              ...
              <div className="px-4 py-3 border-t flex items-center justify-end gap-2">
                <Button type="button" size="sm" variant="outline" onClick={handleCloseClick}>Cancel</Button>
