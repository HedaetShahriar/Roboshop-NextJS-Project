...
type ColumnKey = 'order' | 'customer' | 'payment' | 'status' | 'created' | 'total' | 'billing' | 'actions';
...
interface OrderData {
  ...
  amounts?: {
    total?: number;
    shipping?: number;
    discount?: number | { amount?: number; value?: number };
  };
  status?: string;
  payment?: {
    method?: string;
...
export default async function OrdersTable({ sp = {}, readOnly = false, basePath = "/dashboard/seller/orders" }: OrdersTableProps): Promise<React.ReactElement> {
  ...
  const allColsDefault: ColumnKey[] = ['order', 'customer', 'payment', 'status', 'created', 'total', 'billing', 'actions'];
  let visibleColsArray: ColumnKey[] = (
    colsParam
      ? colsParam.split(',').map(s => s.trim()).filter(Boolean) as ColumnKey[]
      : allColsDefault
  );
  if (readOnly) {
    ...
    <div className="flex flex-col min-h-0 gap-2">
      <div className="shrink-0">
        <TableFilters
          ...
          counts={counts}
          countsUrl={`/api/seller/orders/counts?${new URLSearchParams(queryBase as Record<string, string>).toString()}`}
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-44 p-2 rounded-xl shadow-lg border bg-white">
                  <ul>
                    ...
                    <li>
                      <Link href={`/api/seller/orders/export?${new URLSearchParams({ ...queryBase as Record<string, string>, page: String(page), pageSize: String(pageSize) }).toString()}`} className="rounded px-2 py-1 hover:bg-zinc-50 text-xs inline-flex items-center gap-1"><FileDown size={12} /> Current page</Link>
                    </li>
                  </ul>
                </DropdownMenuContent>
              </DropdownMenu>
      ...
      {!readOnly && (
        <div className="hidden" aria-hidden="true">
          {(orders as unknown as OrderData[]).map((o) => (
            <ToastForm
              key={o._id}
