...
export default function BillingDropdown({ order }: BillingDropdownProps) {
  const id = order?._id;
  const billing: BillingAddress = order?.billingAddress || {};
    ...
    <DropdownMenu open={open} onOpenChange={setOpen}>
      ...
      <DropdownMenuContent align="start" className="w-[min(92vw,380px)] p-3 rounded-xl shadow-lg border bg-white">
        <div className="space-y-3">
          ...
          <form id={formId} action={editBillingAddress as unknown as (formData: FormData) => void} className="grid grid-cols-1 gap-2">
