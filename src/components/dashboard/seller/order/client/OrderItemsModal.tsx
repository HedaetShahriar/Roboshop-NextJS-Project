...
export default function OrderItemsModal({ order }: OrderItemsModalProps) {
  const [open, setOpen] = useState(false);
  const items: OrderItem[] = Array.isArray(order?.items) ? order.items : [];
  const amounts = order?.amounts || {};
  const discountAmount = typeof amounts?.discount === 'number'
    ? Number(amounts.discount)
    : Number((amounts?.discount as Discount)?.amount || (amounts?.discount as Discount)?.value || 0);
  const discountPercent = typeof amounts?.discount === 'object' && (amounts?.discount as Discount)?.type === 'percent'
    ? Number((amounts.discount as Discount).value)
    : null;
  ...
  const billing: Address = order?.billingAddress && Object.keys(order.billingAddress).length > 0
  ...
  const shipping: Address = order?.shippingAddress || {};
  ...
  const addrLine = (a: Address | undefined): string => [a?.address1, a?.city, a?.state, a?.postalCode, a?.country].filter(Boolean).join(", ");
          ...
          <div className="mt-4 grid grid-cols-2 gap-2 text-sm">
            ...
            {Number(amounts?.discount || 0) > 0 && (
              <>
                ...
                <div className="text-right tabular-nums text-emerald-700">- {currencyFmt.format(Number(typeof amounts?.discount === 'number' ? amounts.discount : 0))}</div>
              </>
            )}
