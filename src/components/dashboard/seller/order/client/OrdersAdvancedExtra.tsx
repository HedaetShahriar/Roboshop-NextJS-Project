    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
      <div className="flex flex-col gap-1">
        ...
        <select
          ...
          value={paymentStatus}
          onChange={(e: ChangeEvent<HTMLSelectElement>) => setFilters({ paymentStatus: e.target.value })}
        >
      ...
      <div className="flex flex-col gap-1">
        ...
        <select
          className="h-9 w-full rounded-md border px-2 bg-white"
          value={paymentMethod}
          onChange={(e: ChangeEvent<HTMLSelectElement>) => setFilters({ paymentMethod: e.target.value })}
        >
          {methodOptions.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
        </select>
      </div>
      <label className="inline-flex items-center gap-2 text-xs">
        <input type="checkbox" checked={codOnly} onChange={(e: ChangeEvent<HTMLInputElement>) => setFilters({ codOnly: e.target.checked ? '1' : '' })} /> COD only
      </label>
      <div className="flex flex-col gap-1">
        ...
        <input
          ...
          onChange={(e: ChangeEvent<HTMLInputElement>) => setFilters({ city: e.target.value })}
        />
      </div>
    </div>
  );
