...
interface OrdersBulkActionsPanelProps {
  formId?: string;
  defaultScope?: string;
  pageCount?: number;
  total?: number;
...
interface BulkActionEvent extends Event {
  detail?: {
    action?: string;
    scope?: string;
    rider?: string;
...
export default function OrdersBulkActionsPanel({ formId = "bulkOrdersForm", defaultScope = 'selected', pageCount = 0, total = 0 }: OrdersBulkActionsPanelProps) {
  ...
  useEffect(() => {
    ...
    if (!form) return;
    const ensure = (name: string): HTMLInputElement => {
  ...
  useEffect(() => {
    const form = document.getElementById(formId) as HTMLFormElement | null;
    if (!form) return;
    const aVal = (form.querySelector('input[type="hidden"][name="bulkAction"]') as HTMLInputElement | null)?.value;
    const sVal = (form.querySelector('input[type="hidden"][name="scope"]') as HTMLInputElement | null)?.value;
    const rVal = (form.querySelector('input[type="hidden"][name="bulkRiderName"]') as HTMLInputElement | null)?.value;
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      const detail = (e as BulkActionEvent).detail || {};
  ...
  const onSubmitClick = (e: MouseEvent<HTMLButtonElement>): boolean => {
    ...
    <div className="rounded-md border bg-white p-2">
      <div className="grid gap-1.5">
        ...
        <div className="flex items-center gap-2">
          ...
          <select name="bulkAction" className="h-7 rounded border px-2 text-[12px]" value={action} onChange={(e: ChangeEvent<HTMLSelectElement>) => setAction(e.target.value)} form={formId}>
        ...
        {action === 'assign' && (
          <div className="flex items-center gap-2">
            ...
            <input name="bulkRiderName" placeholder="Rider name" className="h-7 px-2 rounded border text-[12px] sm:w-full w-40" form={formId} value={rider} onChange={(e: ChangeEvent<HTMLInputElement>) => setRider(e.target.value)} />
          </div>
        )}
