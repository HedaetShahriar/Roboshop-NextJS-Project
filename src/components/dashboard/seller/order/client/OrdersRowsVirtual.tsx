...
interface OrdersRowsVirtualProps {
  orders?: Order[];
  visibleCols?: string[];
...
export default function OrdersRowsVirtual({ orders = [], visibleCols = ['order','customer','status','created','total','actions'], density = 'cozy', sortKey = 'newest', query = {}, basePath = '/dashboard/seller/orders' }: OrdersRowsVirtualProps) {
    ...
    <div className="hidden sm:flex min-h-0 flex-1 flex-col rounded border bg-white">
      ...
      <div ref={containerRef} className="min-h-0 flex-1 overflow-auto">
        <div style={{ paddingTop: padTop, paddingBottom: padBottom }}>
          {orders.slice(startIdx, endIdx).map((o) => (
...
function badgeCls(status: string | undefined): string {
  switch (status) {
