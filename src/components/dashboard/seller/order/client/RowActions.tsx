...
export default function RowActions({ id, currentStatus, contact, shipping, tracking, shippingFee: initialShippingFee, discount }: RowActionsProps) {
  ...
  const rememberRider = useCallback((name: string) => {
  ...
  const submitStatus = useCallback((next: string) => {
  ...
  const openBulkWith = useCallback((nextAction: string, nextScope = 'selected') => {
    // Ensure the current row is selected if scope is selected
    if (nextScope === 'selected') {
      const checkbox = document.querySelector(`input[type="checkbox"][name="ids"][value="${id}"]`) as HTMLInputElement | null;
      if (checkbox && !checkbox.checked) checkbox.click();
    }
    // Set hidden inputs pre-emptively so the bulk panel can read initial state
    const form = document.getElementById('bulkOrdersForm') as HTMLFormElement | null;
    if (form) {
      const setHidden = (name: string, val: string) => {
        let el = form.querySelector(`input[type="hidden"][name="${name}"]`) as HTMLInputElement | null;
  ...
  const discountLabel = useCallback((): string | null => {
    ...
    const discountMode = discount.mode || (discount.type === 'percent' ? 'percent' : (discount.type === 'amount' ? 'amount' : undefined));
    if (!amt) return null;
    return discountMode === 'percent' ? `${amt}%` : formatBDT(amt);
  }, [discount]);
        ...
        <DialogContent className="w-[min(92vw,460px)] text-[12px]">
          ...
          <div className="grid gap-1 text-[12px]">
            ...
            <div className="grid grid-cols-2 gap-1">
              ...
              <button type="button" className="col-span-1 h-8 px-2 rounded border bg-white hover:bg-zinc-50 inline-flex items-center gap-2" onClick={() => { if (!window.confirm('Clear discount for this order?')) return; start(async () => { const fd = new FormData(); fd.set('id', id); const res = await clearOrderDiscount(fd) as ActionResult; res?.ok ? toast.success(res.message) : toast.error(res.message || 'Failed to clear'); }); }}><XCircle size={14} /> Clear discount</button>
              <button type="button" className="col-span-1 h-8 px-2 rounded border bg-white hover:bg-zinc-50 inline-flex items-center gap-2" onClick={() => { if (!window.confirm('Clear shipping fee for this order?')) return; start(async () => { const fd = new FormData(); fd.set('id', id); const res = await clearOrderShipping(fd) as ActionResult; res?.ok ? toast.success(res.message) : toast.error(res.message || 'Failed to clear'); }); }}><XCircle size={14} /> Clear shipping</button>
          ...
          <DialogFooter>
            ...
            <Button type="button" size="sm" disabled={pending || !value} onClick={() => {
              ...
              start(async () => {
                ...
                const res = await updateOrderDiscount(fd) as ActionResult;
                if (res?.ok) {
          ...
          <DialogFooter>
            ...
            <Button type="button" size="sm" disabled={pending || !shippingFee} onClick={() => {
              ...
              start(async () => {
                ...
                const res = await updateOrderShipping(fd) as ActionResult;
                if (res?.ok) {
                  toast.success(res.message);
          ...
          <DialogFooter>
            ...
            <Button type="button" size="sm" disabled={pending} onClick={() => {
              start(async () => {
                ...
                const res = await assignOrderRider(fd) as ActionResult;
                if (res?.ok) {
                  toast.success(res.message);
                  setAssignOpen(false);
                  setRiderName('');
