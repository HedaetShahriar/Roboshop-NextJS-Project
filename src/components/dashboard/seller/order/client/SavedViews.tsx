...
const STORAGE_KEY = "ordersSavedViews";
...
export default function SavedViews() {
  ...
  const saveCurrent = () => {
    ...
    const next: SavedView[] = [{ id: Date.now(), name, qs }, ...views].slice(0, 20);
    setViews(next);
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(next)); } catch {}
    setNameInput("");
  };

  const overwriteView = (id: number) => {
    const qs = window.location.search || '';
  ...
  const applyView = (v: SavedView) => {
    const base = '/dashboard/seller/orders';
  ...
  const getPreviewChips = (qs: string): string[] => {
    if (!qs) return [];
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 px-1">
        <Input value={nameInput} onChange={(e: ChangeEvent<HTMLInputElement>) => setNameInput(e.target.value)} placeholder="View name" className="h-7 px-2 text-[12px]" />
              ...
              <div className="flex items-center gap-2">
                ...
                {renameId === v.id ? (
                  <div className="flex items-center gap-2 flex-1">
                    <Input value={renameInput} onChange={(e: ChangeEvent<HTMLInputElement>) => setRenameInput(e.target.value)} className="h-7 px-2 text-[12px]" />
