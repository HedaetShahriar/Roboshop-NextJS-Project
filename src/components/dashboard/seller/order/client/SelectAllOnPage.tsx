...
export default function SelectAllOnPage() {
  ...
  const toggle = () => {
    const next = !checked;
    setChecked(next);
    const inputs = document.querySelectorAll('input[type="checkbox"][name="ids"]') as NodeListOf<HTMLInputElement>;
    inputs.forEach((el) => { el.checked = next; el.dispatchEvent(new Event('change', { bubbles: true })); });
  };
  return (
