...
export default function SelectedBar({ formId = "bulkOrdersForm" }: SelectedBarProps) {
  ...
  useEffect(() => {
    ...
    if (!form) return;
    const onChange = () => {
      const boxes = form.querySelectorAll('input[type="checkbox"][name="ids"]') as NodeListOf<HTMLInputElement>;
