...
type SortKey = 'newest' | 'oldest' | 'amount-high' | 'amount-low' | 'status-asc' | 'status-desc' | string;
...
export default async function OrdersRows({ 
  orders = [], 
  visibleCols = ['order','customer','status','created','total','actions'], 
  density = 'cozy', 
  sortKey = 'newest', 
  query = {}, 
  ...
  tableHeight = 560, 
  readOnly = false 
}: OrdersRowsProps): Promise<React.ReactElement> {
  ...
  const allColKeys: ColumnKey[] = ['order','customer','payment','status','created','total','billing','actions'];
  const colCount = 1 + allColKeys.reduce((n, k) => n + (cols.has(k) ? 1 : 0), 0); // 1 for checkbox column

  const createdSortDir: SortDirection = (sortKey === 'oldest') ? 'ascending' : (sortKey === 'newest' ? 'descending' : 'none');
  const totalSortDir: SortDirection = (sortKey === 'amount-low') ? 'ascending' : (sortKey === 'amount-high' ? 'descending' : 'none');

  const sortHref = (next: string): string => {
            ...
            <tr key={o._id} className={(density === 'compact' ? 'border-t odd:bg-zinc-50/40' : 'border-t odd:bg-zinc-50/40') + ''} data-row-idx={i}>
              ...
              {cols.has('total') && (
                <td className={"px-3 " + (density === 'compact' ? 'py-1.5' : 'py-2') + ' text-right'}>
                  ...
                  {(() => {
                    const discountVal = o?.amounts?.discount;
                    const hasDisc = typeof discountVal === 'number' ? discountVal > 0 : Boolean((discountVal as { amount?: number; value?: number })?.amount || (discountVal as { amount?: number; value?: number })?.value);
