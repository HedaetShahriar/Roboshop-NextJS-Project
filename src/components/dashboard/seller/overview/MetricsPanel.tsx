...
export default function MetricsPanel(): React.ReactElement {
  const [days, setDays] = useState<number>(30);
  const [loading, setLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [data, setData] = useState<MetricsData>({ series: { revenue: [], orders: [] }, totals: { revenue: 0, orders: 0 }, range: null });
  ...
  const fmtInteger = useMemo(() => (n: number): string => new Intl.NumberFormat(undefined, { maximumFractionDigits: 0 }).format(n), []);
