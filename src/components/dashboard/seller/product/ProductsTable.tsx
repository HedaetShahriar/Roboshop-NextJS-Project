...
import BulkActionsPanel, { type Scope } from "./client/BulkActionsPanel";
...
export default async function ProductsTable(props: ProductsTableProps) {
  const sp: SearchParams = props?.sp ?? props?.params ?? {};
  ...
  const mkQS = (overrides: Record<string, string | number | undefined> = {}): string => buildQuery({
    search: q || undefined,
...
async function StatsChips({ sp }: StatsChipsProps) {
...
async function BulkCountsHole({ sp, scopeParam, page, pageSize }: BulkCountsHoleProps) {
  ...
  // Validate scopeParam to be a valid Scope value
  const validScopes: Scope[] = ['selected', 'page', 'filtered'];
  const resolvedScope: Scope = validScopes.includes(scopeParam as Scope) ? scopeParam as Scope : 'selected';
  return (
    <BulkActionsPanel
      compact
...
async function PaginationHole({ sp, page, pageSize, q, fromStr, toStr, sortKey, colsParam, basePath = "/dashboard/seller/products" }: PaginationHoleProps) {
  const totals = await getProductsFilteredTotals(sp);
  const total = Number(totals?.count || 0);
  return (
    <PaginationServer
...
async function TotalsFooter({ sp }: TotalsFooterProps) {
  const totals = await getProductsFilteredTotals(sp);
  return (
