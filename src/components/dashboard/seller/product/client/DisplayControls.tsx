...
export default function DisplayControls({ basePath, allCols, visibleCols, query }: DisplayControlsProps) {
  // Recreate Set on the client from a serializable array
  const visibleSet = new Set(visibleCols || []);

  const mkQS = (overrides: QueryParams = {}): string => {
    ...
    <div>
      <div>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2 px-1">
          {(() => {
            const preset = (colsArr: string[]) => `${basePath}${mkQS({ cols: colsArr.join(','), page: 1 })}`;
