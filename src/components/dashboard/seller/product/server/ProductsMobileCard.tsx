...
interface ProductsMobileCardProps {
  p: Product;
  id: string;
  adjustStockAction: ServerAction;
  updatePricingAction: ServerAction;
  clearDiscountAction: ServerAction;
  setVisibilityAction: ServerAction;
...
export default function ProductsMobileCard({ 
  ...
  const categoryVal = p.category ?? pAny.categoryName ?? pAny.category_name ?? pAny.category_title ?? pAny.categoryLabel;
  const subcategoryVal = p.subcategory ?? pAny.subCategory ?? pAny.sub_category ?? pAny.subcategoryName ?? pAny.subcategory_name ?? pAny.subCategoryName ?? pAny.subcategoryLabel;
        ...
        <div className="min-w-0 flex-1">
          ...
          {(categoryVal || subcategoryVal) ? (
            <div className="mt-1 text-[11px] text-muted-foreground truncate">
              <span>{String(categoryVal) || '—'}</span>
              {subcategoryVal ? <span className="mx-1">/</span> : null}
              {subcategoryVal ? <span>{String(subcategoryVal)}</span> : null}
            </div>
          ) : null}
