...
export default async function ProductsRows({ sp, visibleColsArray, formId = "bulkProductsForm" }: ProductsRowsProps) {
    ...
    <>
      ...
      <div className="sm:hidden space-y-2 overflow-auto">
        {products.map((p: Product) => {
          const id = typeof p._id === 'object' && p._id !== null && 'toString' in p._id 
            ? p._id.toString() 
            : String(p._id);
          return (
            <ProductsMobileCard 
