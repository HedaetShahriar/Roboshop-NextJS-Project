...
async function getActingContext(): Promise<ActingContext> {
...
function asOwnerFilter(filter: Filter<Document> | null, impersonateSellerId?: string): Filter<Document> | null {
  // If super admin provides impersonateSellerId, enforce sellerId on write filters
  if (!impersonateSellerId) return filter;
  if (!filter) return { sellerId: impersonateSellerId };
...
export async function adjustStock(prevState: ActionResult | null, formData: FormData): Promise<ActionResult> {
  // Support both direct form action (formData only) and useActionState(prev, formData)
  const { role } = await getActingContext();
  ...
  let _id: ObjectId;
  try { _id = new ObjectId(String(id)); } catch { return { ok: false }; }
...
export async function updatePricing(prevState: ActionResult | null, formData: FormData): Promise<ActionResult> {
  const { role } = await getActingContext();
  if (role === 'customer') return { ok: false };
  const id = formData.get('id');
...
export async function clearDiscountSingle(prevState: ActionResult | null, formData: FormData): Promise<ActionResult> {
  const { role } = await getActingContext();
  if (role === 'customer') return { ok: false };
  const id = formData.get('id');
...
export async function setVisibility(prevState: ActionResult | null, formData: FormData): Promise<ActionResult> {
...
export async function deleteSingle(prevState: ActionResult | null, formData: FormData): Promise<ActionResult> {
  const { session, role } = await getActingContext();
  if (role === 'customer') return { ok: false };
  const id = formData.get('id');
...
export async function updateProductDetails(prevState: ActionResult | null, formData: FormData): Promise<ActionResult> {
  const { session, role } = await getActingContext();
  if (role === 'customer') return { ok: false };
  const id = formData.get('id');
  ...
  if (discountPrice !== undefined) {
    ...
    if (hasDiscount) {
      ...
      if (update.price && d >= (update.price as number)) return { ok: false, message: 'Discount must be less than price' };
  ...
  const parseArrayOfObjects = (prefix: string, keys: string[]): Record<string, string>[] => {
...
export async function bulkProducts(formData: FormData): Promise<void> {
  const { session, role } = await getActingContext();
  if (role === 'customer') return;
  const action = formData.get('bulkAction');
  const impersonateSellerId = formData.get('impersonateSellerId')?.toString() || '';
  const scopeRaw = (formData.get('scope') || 'selected').toString();
  const scope = (['single', 'bulk', 'page', 'filtered'].includes(scopeRaw) ? scopeRaw : 'single') as 'single' | 'bulk' | 'page' | 'filtered';
  const db = await getDb();
  const now = new Date();

  // Build targeting
  let filter: Filter<Document> | null = null;
  let ids: (FormDataEntryValue | string)[] = formData.getAll('ids');
  if (scope === 'page') {
    const s = {
    ...
    ids = pageProducts.map((p: { _id: { toString: () => string } }) => p._id.toString());
  ...
  const idsFilter = (): Filter<Document> | null => {
  ...
  const targetFilter = (): Filter<Document> | null => {
    const base = (filter ? filter : idsFilter());
    return asOwnerFilter(base, impersonateSellerId || (role !== 'admin' ? (session?.user?.id || '') : ''));
  };
