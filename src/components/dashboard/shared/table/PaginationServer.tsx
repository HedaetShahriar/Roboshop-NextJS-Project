...
export default function PaginationServer({ 
...
}: PaginationServerProps): React.ReactElement {
  ...
  const toQS = (overrides: QueryParams = {}): string => {
  ...
  const pageItems: PageItem[] = (() => {
    ...
    const add = (type: 'page' | 'ellipsis', value?: number): void => { items.push({ type, value }); };
    const pushPage = (p: number): void => add("page", p);
  ...
  const sizeOptions: SizeOption[] = (pageSizes || [10,20,50,100]).map((n) => ({
    value: n,
    label: String(n),
