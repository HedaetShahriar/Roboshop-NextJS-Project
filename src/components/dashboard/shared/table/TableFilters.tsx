...
import { useState, useMemo, useEffect, memo, useRef, type RefObject, type ChangeEvent, type FormEvent, type ReactNode } from 'react';
...
function TableFilters({
    ...
    const filters: FilterValues = value ?? fallback.filters;
    ...
    const [countsState, setCountsState] = useState<StatusCounts | undefined>(counts);
    ...
    const [localSort, setLocalSort] = useState<string>(sort || 'newest');
    // Optional product extras
    const [localInStock, setLocalInStock] = useState<boolean>(Boolean(inStock) || false);
    const [localHasDiscount, setLocalHasDiscount] = useState<boolean>(Boolean(hasDiscount) || false);
    const [localLowStock, setLocalLowStock] = useState<boolean>(Boolean(lowStock) || false);
    ...
    const [localSubcategory, setLocalSubcategory] = useState<string>(subcategory || '');
    ...
    const filteredSubcategoryOptions = useMemo((): string[] => {
    ...
    const sortLabelMap = useMemo((): Record<string, string> => {
    ...
    const sortOpts = useMemo((): SortOption[] => (
    ...
    const hasChips = useMemo((): boolean => {
    ...
    // Date preset helpers
    const applyDatePreset = (key: string): void => {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (!res.ok) return;
                const data: StatusCounts = await res.json();
    ...
    return (
                                        ...
                                        <div className="flex flex-col gap-1">
                                            ...
                                            {Array.isArray(categoryOptions) && categoryOptions.length ? (
                                                <select
                                                    id="category-dd"
                                                    value={localCategory}
                                                    onChange={(e: ChangeEvent<HTMLSelectElement>) => setLocalCategory(e.target.value)}
                                                ...
                                                <Input id="category-dd" type="text" value={localCategory} onChange={(e: ChangeEvent<HTMLInputElement>)=>setLocalCategory(e.target.value)} className="h-9 w-full" />
                                            )}
                                        </div>
                                    )}
                                    {cfg.showSubcategory && (
                                        <div className="flex flex-col gap-1">
                                            ...
                                            {Array.isArray(filteredSubcategoryOptions) && filteredSubcategoryOptions.length ? (
                                                <select
                                                    ...
                                                    onChange={(e: ChangeEvent<HTMLSelectElement>) => setLocalSubcategory(e.target.value)}
                                                ...
                                                <Input id="subcategory-dd" type="text" value={localSubcategory} onChange={(e: ChangeEvent<HTMLInputElement>)=>setLocalSubcategory(e.target.value)} disabled={disableSubcategory} aria-disabled={disableSubcategory} placeholder={disableSubcategory ? 'Select category first' : undefined} className="h-9 w-full disabled:opacity-60 disabled:cursor-not-allowed" />
                                            )}
                                            {disableSubcategory && (
                                    ...
                                    <div className="flex items-center justify-end gap-2 mt-1">
                                        ...
                                        <Button onClick={() => {
                                            ...
                                            if (cfg.showCategory && cfg.showSubcategory) {
                                                const validSubs: string[] = (localCategory && CATEGORY_MAP && CATEGORY_MAP[localCategory]) ? CATEGORY_MAP[localCategory] : [];
                                        ...
                                        <div className="flex flex-wrap items-center gap-3">
                                            {cfg.showInStock && (
                                                <label className="inline-flex items-center gap-2 text-xs">
                                                    <input type="checkbox" checked={!!localInStock} onChange={(e: ChangeEvent<HTMLInputElement>)=>setLocalInStock(e.target.checked)} /> <span className="inline-flex items-center gap-1"><Package size={12} /> In stock only</span>
                                                </label>
                                            )}
                                            {cfg.showHasDiscount && (
                                                <label className="inline-flex items-center gap-2 text-xs">
                                                    <input type="checkbox" checked={!!localHasDiscount} onChange={(e: ChangeEvent<HTMLInputElement>)=>setLocalHasDiscount(e.target.checked)} /> <span className="inline-flex items-center gap-1"><Percent size={12} /> Has discount</span>
                                                </label>
                                            )}
                                            {cfg.showLowStock && (
                                                <label className="inline-flex items-center gap-2 text-xs">
                                                    <input type="checkbox" checked={!!localLowStock} onChange={(e: ChangeEvent<HTMLInputElement>)=>setLocalLowStock(e.target.checked)} /> <span className="inline-flex items-center gap-1"><AlertTriangle size={12} /> Low stock</span>
                                                </label>
                                            )}
                                        </div>
                                    )}
                                    ...
                                    {cfg.dateRange && (
                                        <div className="flex flex-col gap-1">
                                            ...
                                            <DateRangePicker
                                                from={localFrom}
                                                to={localTo}
                                                onChange={({ from, to }: { from?: string; to?: string }) => { setLocalFrom(from || ''); setLocalTo(to || ''); }}
                                            />
                                        </div>
                                    )}
                                    {cfg.showPriceRange && (
                                        <div className="grid grid-cols-2 gap-3">
                                            <div className="flex flex-col gap-1">
                                                ...
                                                <Input id="minPrice" type="number" value={localMinPrice} onChange={(e: ChangeEvent<HTMLInputElement>)=>setLocalMinPrice(e.target.value)} className="h-9 w-full" />
                                            </div>
                                            <div className="flex flex-col gap-1">
                                                ...
                                                <Input id="maxPrice" type="number" value={localMaxPrice} onChange={(e: ChangeEvent<HTMLInputElement>)=>setLocalMaxPrice(e.target.value)} className="h-9 w-full" />
                                            </div>
                                        </div>
                                    )}
                                    {cfg.sort && (
                                        <div className="flex flex-col gap-1">
                                            ...
                                            <div className="relative">
                                                <select id="sort" value={localSort || (sortOpts[0]?.value || 'newest')} onChange={(e: ChangeEvent<HTMLSelectElement>) => setLocalSort(e.target.value)} className="block w-full h-9 rounded-md border pr-8 px-3 text-sm bg-white">
                                                    {sortOpts.map((o) => (
        ...
        <div className={"grid grid-cols-1 " + (compact ? "gap-2" : "gap-3")}>
                {cfg.search && (
            <form className={"flex items-center w-full " + (compact ? "gap-2" : "gap-3")} onSubmit={(e: FormEvent<HTMLFormElement>) => { 
                        e.preventDefault(); 
                ...
                <div className="flex items-center gap-2 overflow-x-auto flex-nowrap [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
                    {Boolean(search) && (
                        <button type="button" className="h-7 px-2 rounded-full border text-xs bg-white hover:bg-zinc-50 flex items-center gap-1" onClick={()=>setFilters({ search: '' })}>
                            Search: &ldquo;{search}&rdquo; <X size={12} />
                        </button>
                    )}
...
function KeyboardShortcuts({ searchRef }: KeyboardShortcutsProps): null {
    useEffect(() => {
        ...
        const onKey = (e: KeyboardEvent): void => {
            ...
            if ((e.key === 'a' || e.key === 'A') && !e.ctrlKey && !e.metaKey && !isTyping) {
                const boxes = document.querySelectorAll<HTMLInputElement>('form#bulkOrdersForm input[type="checkbox"][name="ids"]');
                if (boxes.length) {
                    e.preventDefault();
        ...
        const onClick = (e: MouseEvent): void => {
            ...
            if (target && target.matches('form#bulkOrdersForm input[type="checkbox"][name="ids"]')) {
                if (e.shiftKey && lastChecked && lastChecked !== target) {
                    const boxes = Array.from(document.querySelectorAll<HTMLInputElement>('form#bulkOrdersForm input[type="checkbox"][name="ids"]'));
                    const start = boxes.indexOf(lastChecked);
                    const end = boxes.indexOf(target as HTMLInputElement);
                    if (start !== -1 && end !== -1) {
                ...
                lastChecked = target as HTMLInputElement;
            }
        };
