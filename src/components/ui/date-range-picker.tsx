...
export default function DateRangePicker({
  ...
  const monthLabel = useMemo(() => {
    return new Date(viewYear, viewMonth, 1).toLocaleString("en-US", {
      month: "short",
      year: "numeric",
    });
  }, [viewYear, viewMonth]);
  ...
  const onDayClick = (d: Date | null): void => {
    if (!d) return;
    if (!fromDate && !toDate && !pendingStart) {
      setPendingStart(d);
      if (onChange) onChange({ from: fmtYMD(d), to: to || "" });
      return;
    }
    ...
    <div className="flex flex-col gap-1">
      <div className="flex items-center justify-between">
        <button
          type="button"
          onClick={handlePrev}
          className="h-6 w-6 text-[10px] inline-flex items-center justify-center rounded border hover:bg-zinc-50"
        >
          ‹
        </button>
      ...
      <div className="flex items-center justify-between text-[10px]">
        ...
        <button
          type="button"
          onClick={clear}
          className="h-6 px-2 rounded border text-[11px] bg-white hover:bg-zinc-50"
        >
          Clear
        </button>
      </div>
