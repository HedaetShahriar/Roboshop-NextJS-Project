...
export function buildProductsWhere(
  ...
  const lowStock = (sp as Record<string, unknown>)?.lowStock ? true : false;
  const minPrice =
    sp?.minPrice !== undefined && sp?.minPrice !== ""
      ? Number(sp.minPrice)
      : undefined;
  const maxPrice =
    sp?.maxPrice !== undefined && sp?.maxPrice !== ""
      ? Number(sp.maxPrice)
      : undefined;
...
export function getProductsSort(sortKey: string = "newest"): Sort {
  if (sortKey === "category-asc")
    return { category: 1, name: 1, createdAt: -1 };
  if (sortKey === "category-desc")
    return { category: -1, name: 1, createdAt: -1 };
  if (sortKey === "subcategory-asc")
    return { subcategory: 1, name: 1, createdAt: -1 };
  if (sortKey === "subcategory-desc")
    return { subcategory: -1, name: 1, createdAt: -1 };
  ...
  if (sortKey === "rating-high")
    return { product_rating: -1, product_rating_count: -1, createdAt: -1 };
  if (sortKey === "rating-low")
    return { product_rating: 1, product_rating_count: -1, createdAt: -1 };
  return { createdAt: -1 };
}

export function getPageAndSize(sp: ProductFilters = {}): {
  page: number;
  pageSize: number;
} {
